...
class TransactionHistoryList extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(IterableProperty<TransactionHistoryItem>(
...
class _TransactionHistoryListState extends State<TransactionHistoryList> {
  ...
  void initState() {
    super.initState();
    groupedHistory.addAll(widget.transactionsList
        .groupBy((m) => formatDate(m.date, DateFormatEnum.groupByMonthYear)));
  }
            ...
            child: Column(
              ...
              children: [
                TransactionHistoryGroupTitle(
                    title: extractMonth(groupedMonthYear)),
                ...transactionsList
                    .map(
                      (transaction) => TransactionHistoryCard(
                        transactionType: transaction.transactionType,
                        denomText: transaction.amount
                            .map((e) =>
                                e?.denom.split('').take(8).join() ?? '')
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(
        DiagnosticsProperty<Map<String, List<TransactionHistoryItem>>>(
