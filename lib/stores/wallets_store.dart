...
class WalletsStore {
  ...
  static const chainId = 'cosmoshub-testnet';
  ...
  WalletPublicInfo get selectedWallet {
    ...
    if (index == null) {
      return const WalletPublicInfo(
          chainId: '', name: '', publicAddress: '', walletId: '');
    }
  ...
  Future<void> getBalances(String walletAddress) async {
    ...
    try {
      final val = await CosmosBalances(baseEnv).getBalances(walletAddress);
      for (var it in val) {
        String baseDenom;
        try {
          baseDenom =
  ...
  Future<String?> createMnemonic(
      [VoidCallback? onMnemonicGenerationStarted]) async {
